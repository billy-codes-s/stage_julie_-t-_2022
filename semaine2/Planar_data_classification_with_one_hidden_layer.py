...
def compute_cost(A2, Y):
    ...
    # YOUR CODE ENDS HERE
    ## not quite.. more elegant, I could of made a single line of code
    ## cost  =...np.sum(y * np.log(A2)...)
    ## take note of this np.squeeze, transforms it in a "dimension we expect" as if that meant something LOL
...
def backward_propagation(parameters, cache, X, Y):
    ...
    db2 = coeff * np.sum(dZ2, axis = 1, keepdims = True)
    dZ1 = np.matmul(W2.T, dZ2) * (1-A1**2)
    dW1 = coeff * np.matmul(dZ1, X.T)
    db1 = coeff * np.sum(dZ1, axis = 1,keepdims = True)
...
def nn_model(X, Y, n_h, num_iterations = 10000, print_cost=False):
    ...
    for i in range(0, num_iterations):
        ...
        ## compute the data necessary to find the gradient
